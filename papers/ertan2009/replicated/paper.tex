...
\author{
  ...
  \and
  Claude\\
  Anthropic (\texttt{claude-opus-4-6})
}
...
\begin{abstract}
We replicate the public goods experiment of \citet{ertan2009who}, in which participants decide how much to contribute to a group project and vote on whether to allow punishment of low or high contributors, using Large Language Model (LLM) agents connected to a real \texttt{oTree} experimental server. We compare three conditions at $N=50$ each: (i) default LLM agents with no personality assignment, (ii) agents assigned Big Five personality traits drawn from US adult norms via the BFI-2-Expanded sentence format, and (iii) agents drawn from a distribution skewed toward high agreeableness and low neuroticism (``hyper-cooperative''). Default agents reproduce the key qualitative finding of the original paper---$94\%$ vote to allow punishment of low contributors (original paper: $64\%$ of all group votes across both designs, with ``nearly all groups'' in the final vote; see Section~\ref{sec:paper-comparison}), $0\%$ vote to allow punishment of high contributors (paper: also $0\%$)---and engage in substantial costly punishment of free-riders ($M=3.78$ of $5$ tokens; $94\%$ non-zero). Random-personality agents contribute far more at baseline ($M=12.3$ vs.\ $1.0$, Mann--Whitney $p<.001$) and respond more strongly to the punish-low regime ($M=17.8$), but spend significantly less on costly punishment ($M=1.90$ on free-riders vs.\ $3.78$, $p<.001$) and are less inclined to vote punitive institutions ($80\%$ vs.\ $94\%$). The hyper-cooperative condition amplifies this drift on the pages that do complete: baseline contribution rises to $M=16.5$ and support for allowing punishment of low contributors collapses to $46\%$ ($\chi^2=25.19$, $p<.001$ vs.\ default). Separately, $43$ of $50$ hyper-cooperative agents fail to produce any content on the costly-punishment page. On inspection, every single failed attempt is an \emph{empty-content} response from the model's API, and the comparable pages complete $50/50$ in both other conditions on the same pipeline. We trace this to the fact that the underlying model, \texttt{stepfun/step-3.5-flash}, is a reasoning model that separates internal reasoning tokens from user-visible content tokens: the hyper-cooperative personality prompt induces enough deliberation on the ethically loaded punishment questions that the $16{,}000$-token budget is exhausted on reasoning before any content token is emitted. This is a personality-driven effect (it does not occur in either other condition) but a model- and budget-specific one, and we treat it accordingly. Across all three conditions, agents reject the punish-high-only regime and, when they do punish, target the lowest contributor. Our main positive result is that \emph{personality induction} shifts the cooperation--punishment trade-off in the direction predicted by RLHF prosociality bias, in a dose-dependent way across the three conditions.
...
This architecture is, as it turns out, not a cosmetic detail. The same base model placed in a bare one-shot survey context versus a structured oTree form produces dramatically different behavior on the same questions. We return to this point in Section~\ref{sec:discussion}.
